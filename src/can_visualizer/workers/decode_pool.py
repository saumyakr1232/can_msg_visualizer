...
class DecodeTask:
    ...
    batch_id: int
    messages: (
        list  # List of (timestamp, arbitration_id, data_bytes, is_extended_id, channel)
    )
...
def _decode_batch(task_data: tuple) -> DecodeResult:
    ...
    return DecodeResult(
        batch_id=batch_id, signals=decoded_signals, error_count=error_count
    )
...
class DecodePool:
    ...
    def __init__(
        ...
        logger.info(
            f"DecodePool initialized with {max_workers} workers, batch_size={batch_size}"
        )
    ...
    def decode_messages(
        self, messages: Iterator[tuple], progress_callback: Optional[callable] = None
    ) -> Iterator[list]:
        """
