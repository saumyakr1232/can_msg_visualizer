...
class SignalSelectorDialog(QDialog):
    ...
    def _populate_list(self) -> None:
        ...
        sorted_signals = sorted(
            self._signal_defs.items(),
            key=lambda x: (x[1].message_name, x[1].name)
        )
        ...
        for full_name, sig_def in sorted_signals:
            ...
            tooltip += f"Message: {sig_def.message_name} (0x{sig_def.message_id:03X})<br>"
    ...
    def _on_search_changed(self, text: str) -> None:
        ...
        for i in range(self._list_widget.count()):
            ...
            if not search_text:
                ...
                if sig_def:
                    matches = (
                        search_text in sig_def.name.lower() or
                        search_text in sig_def.message_name.lower() or
                        search_text in full_name.lower()
                    )
    ...
    def _update_selection_count(self) -> None:
        ...
        self._selection_label.setText(f"{count} signal{'s' if count != 1 else ''} selected")
