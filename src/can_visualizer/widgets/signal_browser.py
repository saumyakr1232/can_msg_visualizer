...
class SignalBrowserWidget(QWidget):
    ...
    def _on_search_changed(self, text: str) -> None:
        ...
        for msg_id, msg_item in self._message_items.items():
            ...
            msg_matches = (
                not search_text
                or search_text in msg_def.name.lower()
                or search_text in msg_def.hex_id.lower()
            )
    ...
    def _on_item_changed(self, item: QTreeWidgetItem, column: int) -> None:
        ...
        selected = [tuple(name.split(".", 1)) for name in sorted(self._checked_signals)]
    ...
    def get_selected_signals(self) -> list[tuple[str, str]]:
        ...
        """
        return [tuple(name.split(".", 1)) for name in sorted(self._checked_signals)]
