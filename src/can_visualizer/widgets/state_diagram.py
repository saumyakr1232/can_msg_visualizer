...
class StateTimelineRow(QFrame):
    ...
    def __init__(
        self,
        signal_name: str,
        signal_def: Optional[SignalDefinition] = None,
        parent=None,
    ):
    ...
    def get_color_for_value(self, value: float) -> str:
        ...
        if key not in self._value_colors:
            self._value_colors[key] = STATE_COLORS[
                self._color_index % len(STATE_COLORS)
            ]
            self._color_index += 1
    ...
    def paintEvent(self, event) -> None:
        ...
        elided_name = fm.elidedText(
            self.short_name, Qt.TextElideMode.ElideRight, label_rect_width - 16
        )
            ...
            if segment_width > 25:
                ...
                if text_width > segment_width - 6:
                    display_value = fm.elidedText(
                        display_value, Qt.TextElideMode.ElideRight, segment_width - 6
                    )
                    text_width = fm.horizontalAdvance(display_value)
        ...
        if (
            self.cursor_time is not None
            and self.time_min <= self.cursor_time <= self.time_max
        ):
...
class StateDiagramControlPanel(QWidget):
    ...
    def set_signals(self, signal_names: list[str]):
        ...
        self._info_label.setText(
            f"{len(signal_names)} signal{'s' if len(signal_names) != 1 else ''}"
        )
    ...
    def set_running(self, running: bool):
        ...
        self._status_label.setStyleSheet(
            f"color: {'#4CAF50' if running else '#888'}; font-size: 10px;"
        )
    ...
    def set_playback_time(self, current_time: float, total_time: float):
        ...
        if total_time > 0:
            ...
            self._status_label.setText(
                f"▶ {percent:.1f}% ({current_time:.2f}s / {total_time:.2f}s)"
            )
...
class StateDiagramWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        self._control_panel.add_signals_requested.connect(
            self.add_signals_requested.emit
        )
        ...
        self._empty_label = QLabel(
            "Click 'Add Signals' to select signals for the state diagram"
        )
    ...
    def _do_deferred_view_update(self) -> None:
        """Execute deferred view update."""
        self._view_update_pending = False
        self._fit_view_to_data()
